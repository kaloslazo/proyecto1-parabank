...
import { clientFunctionalities, clientRequirements, Finding, initialCases, initialFindings, navItems, NavKey, Priority, requirements, TestCase, TestStatus } from './data'
...
function Overview({ onNavigate }: { onNavigate: (key: NavKey) => void }) {
  return <>
    <PageIntro eyebrow="Vista de avance" title="Primer avance / Requisitos del cliente" description="Caso 1 · ParaBank · funcionalidades completas y requisitos verificables en formato RF." action={<a className="button primary" href="https://parabank.parasoft.com/parabank/index.htm" target="_blank" rel="noreferrer">Abrir ParaBank <Icon name="external" size={16} /></a>} />
    <section className="case-sheet">
      <div className="sheet-intro"><div><span className="sheet-code">PB-01 / AVANCE</span><h2>Requisitos del cliente</h2><p>Primero se identifica todo el alcance del sistema. Después, cada funcionalidad se traduce a requisitos verificables y exponibles.</p></div><dl className="sheet-meta"><div><dt>Equipo</dt><dd>Kalos Lazo + Gianpier Segovia</dd></div><div><dt>Formato</dt><dd>Requisito (RF)</dd></div><div><dt>Estado</dt><dd><span className="live-marker" /> Listo para exposición</dd></div></dl></div>
...
function Functionalities() {
  return <><PageIntro title="Funcionalidades identificadas" description="Mapa completo del alcance funcional de ParaBank. Cada fila abre el bloque de requisitos que se expondrá." action={<Badge tone="mint">{clientFunctionalities.length}/8 cubiertas</Badge>} /><div className="functionality-note"><Icon name="clipboard" size={20} /><div><strong>Formato elegido: Requisito (RF)</strong><p>Se mantiene el mismo formato en las ocho funcionalidades, siguiendo la guía del primer avance.</p></div></div><div className="functionality-table"><div className="functionality-head"><span>ID</span><span>Funcionalidad</span><span>Dónde se encuentra en el sistema real</span><span>Actor</span><span>RF</span></div>{clientFunctionalities.map((item) => <button className="functionality-row" key={item.id} onClick={() => { document.querySelector(`[data-functionality="${item.id}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'start' }) }}><span className="functionality-id">{item.id}</span><div><strong>{item.name}</strong><small>{item.url}</small></div><span>{item.location}</span><span>{item.actor}</span><span className="rf-count">{item.requirementIds.length}</span></button>)}</div><div className="functionality-foot"><span>Alcance completo</span><strong>8 funcionalidades · {clientRequirements.length} requisitos verificables</strong><span>La exposición debe seguir este mismo orden.</span></div></>
}
...
function ClientRequirements() {
  ...
  return <><PageIntro title="Requisitos del cliente" description="Formato RF: cada requisito declara acción, actor, datos y reglas, resultado observable y prioridad." action={<Badge tone="plum">{clientRequirements.length} RF</Badge>} /><div className="rf-layout"><aside className="rf-nav"><span className="micro-label">Funcionalidades</span>{clientFunctionalities.map((item) => <button key={item.id} className={activeFunctionality === item.id ? 'rf-nav-item active' : 'rf-nav-item'} onClick={() => setActiveFunctionality(item.id)}><span>{item.id}</span><strong>{item.name}</strong><small>{item.requirementIds.length} requisitos</small></button>)}</aside><section className="rf-panel" data-functionality={current.id}><div className="rf-panel-head"><div><span className="functionality-id">{current.id}</span><h2>{current.name}</h2><p><strong>Ubicación:</strong> {current.location} · <code>{current.url}</code></p></div><Badge tone="mint">{currentRequirements.length} RF</Badge></div><div className="rf-table-wrap"><div className="rf-table"><div className="rf-head"><span>ID</span><span>Requisito verificable</span><span>Actor</span><span>Datos y reglas</span><span>Resultado esperado</span><span>Prioridad</span></div>{currentRequirements.map((item) => <div className="rf-row" key={item.id}><span className="rf-id">{item.id}</span><p>{item.requirement}</p><span>{item.actor}</span><p>{item.dataRules}</p><p>{item.expected}</p><Badge tone={priorityClass[item.priority]}>{item.priority}</Badge></div>)}</div></div></section></div></>
}
