...
import { clientBlackBoxExamples, clientFunctionalities, clientNonFunctionalRequirements, clientRequirements, Finding, initialCases, initialFindings, navItems, NavKey, Priority, requirements, TestCase, TestStatus } from './data'
...
const evidenceUrl = (name: string) => `${import.meta.env.BASE_URL}evidencia/${name}`
...
function App() {
  ...
  const [cases, setCases] = useState<TestCase[]>(() => loadLocal('parabank-cases-v2', initialCases))
  const [findings, setFindings] = useState<Finding[]>(() => loadLocal('parabank-findings-v2', initialFindings))
  ...
  useEffect(() => { localStorage.setItem('parabank-cases-v2', JSON.stringify(cases)) }, [cases])
  useEffect(() => { localStorage.setItem('parabank-findings-v2', JSON.stringify(findings)) }, [findings])
...
function Overview({ onNavigate }: { onNavigate: (key: NavKey) => void }) {
  return <>
    ...
    <section className="case-sheet">
      <div className="sheet-intro"><div><h2>Requisitos del cliente</h2><p>El alcance se organiza en ocho funcionalidades, con requisitos verificables y ejemplos de prueba de caja negra.</p></div><dl className="sheet-meta"><div><dt>Equipo</dt><dd>Kalos Lazo y Gianpier Segovia</dd></div><div><dt>Formato</dt><dd>Requisito funcional (RF)</dd></div><div><dt>Contenido</dt><dd>RF, RNF y ejemplos de prueba</dd></div></dl></div>
      ...
      <div className="sheet-workspace"><div className="risk-table"><div className="sheet-section-head"><div><span className="micro-label">Mapa de funcionalidades</span><h3>Todo el alcance está cubierto</h3></div><button className="text-button" onClick={() => onNavigate('plan')}>Ver tabla completa <Icon name="arrow" size={16} /></button></div><div className="risk-column-head functionality-column-head"><span>ID</span><span>Funcionalidad y ubicación</span><span>RF</span></div>{clientFunctionalities.map((item) => <div className="risk-table-row functionality-summary-row" key={item.id}><span className="risk-id">{item.id}</span><div><strong>{item.name}</strong><small>{item.location}</small></div><span className="rf-count">{item.requirementIds.length}</span></div>)}</div><aside className="overview-guide"><section className="guide-panel"><div><span className="micro-label">Cobertura</span><h3>22 requisitos funcionales</h3><p>Cada requisito indica actor, datos, reglas, resultado esperado y prioridad.</p><button className="text-button" onClick={() => onNavigate('traceability')}>Revisar requisitos <Icon name="arrow" size={15} /></button></div></section><section className="guide-panel"><div><span className="micro-label">Diseño de prueba</span><h3>Caja negra por requisito</h3><p>Cada RF incluye una técnica posible y un ejemplo con datos observables.</p><button className="text-button" onClick={() => onNavigate('traceability')}>Ver ejemplos <Icon name="arrow" size={15} /></button></div></section></aside></div>
    </section>
  </>
}
...
function Functionalities() {
  return <><PageIntro title="Funcionalidades identificadas" description="Mapa completo del alcance funcional definido para ParaBank." action={<Badge tone="mint">{clientFunctionalities.length} de 8</Badge>} /><div className="functionality-note"><Icon name="clipboard" size={20} /><div><strong>Formato elegido: requisito funcional</strong><p>Las ocho funcionalidades usan la misma estructura verificable.</p></div></div><div className="functionality-table"><div className="functionality-head"><span>ID</span><span>Funcionalidad</span><span>Dónde se encuentra en el sistema real</span><span>Actor</span><span>RF</span></div>{clientFunctionalities.map((item) => <div className="functionality-row" key={item.id}><span className="functionality-id" data-label="ID">{item.id}</span><div className="functionality-title" data-label="Funcionalidad"><strong>{item.name}</strong></div><span className="functionality-location" data-label="Ubicación">{item.location}</span><span className="functionality-actor" data-label="Actor">{item.actor}</span><span className="rf-count" data-label="Requisitos">{item.requirementIds.length}</span></div>)}</div><div className="functionality-foot"><span>Alcance completo</span><strong>8 funcionalidades, {clientRequirements.length} requisitos verificables</strong><span>Cada RF incluye un ejemplo de caja negra.</span></div></>
}
