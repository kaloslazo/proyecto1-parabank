  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {activeNav === 'overview' && <Overview completed={completed} highCases={highCases.length} onNavigate={setActiveNav} />}
        {activeNav === 'plan' && <Plan />}
...
function Overview({ completed, highCases, onNavigate }: { completed: number; highCases: number; onNavigate: (key: NavKey) => void }) {
  return <>
    <PageIntro eyebrow="Vista de avance" title="Caso 1 / Banca digital" description="ParaBank · plan, diseño y ejecución de pruebas para operaciones financieras." action={<a className="button primary" href="https://parabank.parasoft.com/parabank/index.htm" target="_blank" rel="noreferrer">Abrir ParaBank <Icon name="external" size={16} /></a>} />
    <section className="case-sheet">
      <div className="sheet-intro"><div><span className="sheet-code">PB-01</span><h2>Plan de pruebas</h2><p>La pregunta de este caso es simple: ¿el sistema mantiene el mismo dinero antes y después de una operación?</p></div><dl className="sheet-meta"><div><dt>Equipo</dt><dd>Kalos Lazo + Gianpier Segovia</dd></div><div><dt>Sistema</dt><dd>parabank.parasoft.com</dd></div><div><dt>Estado</dt><dd><span className="live-marker" /> Diseño inicial</dd></div></dl></div>
      <div className="sheet-stats"><div><span>Requisitos</span><strong>{requirements.length}/{requirements.length}</strong></div><div><span>Casos diseñados</span><strong>20</strong></div><div><span>Prioridad alta</span><strong>{highCases}</strong></div><div><span>Ejecutados</span><strong>{completed}/20</strong></div></div>
      <div className="sheet-workspace"><div className="risk-table"><div className="sheet-section-head"><div><span className="micro-label">Riesgos que cambian la prioridad</span><h3>Qué vamos a comprobar primero</h3></div><button className="text-button" onClick={() => onNavigate('traceability')}>Matriz completa <Icon name="arrow" size={16} /></button></div><div className="risk-column-head"><span>Riesgo</span><span>Condición de prueba</span><span>Prioridad</span></div>{[{ id: 'R-01', title: 'La transferencia no cuadra', detail: 'El débito y el crédito deben ser iguales.', priority: 'Alta' as Priority }, { id: 'R-02', title: 'El pago excede el saldo', detail: 'No debe procesarse un monto mayor al disponible.', priority: 'Alta' as Priority }, { id: 'R-03', title: 'La sesión se pierde', detail: 'La operación debe dejar un resultado claro.', priority: 'Alta' as Priority }, { id: 'R-04', title: 'El historial no refleja el movimiento', detail: 'La operación debe poder encontrarse por filtro.', priority: 'Media' as Priority }].map((risk) => <div className="risk-table-row" key={risk.id}><span className="risk-id">{risk.id}</span><div><strong>{risk.title}</strong><small>{risk.detail}</small></div><Badge tone={priorityClass[risk.priority]}>{risk.priority}</Badge></div>)}</div><aside className="status-column"><div className="status-block status-dark"><span className="micro-label">Flujo bajo prueba</span><h3>Saldo inicial → operación → saldo final → historial</h3><button className="light-button" onClick={() => onNavigate('execution')}>Ir a ejecución <Icon name="arrow" size={15} /></button></div><div className="status-block"><div className="sheet-section-head"><div><span className="micro-label">Siguiente</span><h3>Para cerrar el avance</h3></div><span className="step-count">03</span></div><div className="compact-actions"><button onClick={() => onNavigate('cases')}><span>01</span>Completar datos <Icon name="arrow" size={14} /></button><button onClick={() => onNavigate('execution')}><span>02</span>Ejecutar casos altos <Icon name="arrow" size={14} /></button><button onClick={() => onNavigate('findings')}><span>03</span>Registrar hallazgos <Icon name="arrow" size={14} /></button></div></div></aside></div>
    </section>
  </>
}
