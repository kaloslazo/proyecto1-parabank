  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand-mark"><span>PB</span><div><strong>Parabank</strong><small>Proyecto 1 · QA Lab</small></div></div>
        ...
        <div className="sidebar-bottom">
          <div className="case-mini"><span className="mini-orb">01</span><div><small>Caso activo</small><strong>Banca digital</strong></div></div>
...
function PageIntro({ eyebrow, title, description, action }: { eyebrow: string; title: string; description: string; action?: ReactNode }) {
  return <div className="page-intro"><div><div className="eyebrow">{eyebrow}</div><h1>{title}</h1><p>{description}</p></div>{action && <div className="intro-action">{action}</div>}</div>
}
...
function Overview({ completed, coverage, passed, highCases, executedHigh, onNavigate }: { completed: number; coverage: number; passed: number; highCases: number; executedHigh: number; onNavigate: (key: NavKey) => void }) {
  return <>
    <PageIntro eyebrow="Vista de avance" title="El banco se prueba desde el riesgo." description="Un tablero de trabajo para convertir el caso ParaBank en evidencia, decisiones y casos ejecutables." action={<a className="button primary" href="https://parabank.parasoft.com/parabank/index.htm" target="_blank" rel="noreferrer">Abrir sistema real <Icon name="external" size={16} /></a>} />
    <section className="hero-grid">
      <div className="hero-surface clay-plum"><div className="hero-kicker"><span className="pulse" /> Avance al 18 de setiembre</div><h2>Los flujos de dinero<br /><em>mandan la prioridad.</em></h2><p>La estrategia parte del impacto de una operación desbalanceada: cada transferencia y pago debe dejar una huella verificable.</p><div className="flow-strip"><span>Cliente</span><i>→</i><span className="flow-highlight">Cuenta</span><i>→</i><span className="flow-highlight">Movimiento</span><i>→</i><span>Historial</span></div></div>
      <div className="hero-surface clay-blue"><div className="surface-heading"><div><span className="micro-label">Foco de riesgo</span><h3>Integridad financiera</h3></div><span className="risk-symbol"><Icon name="shield" size={21} /></span></div><div className="risk-list"><div><span className="risk-index">01</span><span>Débito y crédito deben cuadrar</span><Badge tone="high">Alta</Badge></div><div><span className="risk-index">02</span><span>El pago no debe superar saldo</span><Badge tone="high">Alta</Badge></div><div><span className="risk-index">03</span><span>La sesión debe cerrar con seguridad</span><Badge tone="medium">Media</Badge></div></div><button className="text-button" onClick={() => onNavigate('traceability')}>Ver matriz inicial <Icon name="arrow" size={16} /></button></div>
    </section>
    ...
    <section className="dashboard-lower"><div className="coverage-panel"><div className="section-heading"><div><span className="micro-label">Mapa de cobertura</span><h3>Requisitos que ya tienen camino de prueba</h3></div><button className="text-button" onClick={() => onNavigate('traceability')}>Abrir detalle <Icon name="arrow" size={16} /></button></div>{requirements.slice(0, 6).map((item) => <div className="coverage-row" key={item.id}><div className="coverage-label"><span>{item.id}</span><strong>{item.title}</strong></div><ProgressBar value={100} tone={item.risk === 'Alta' ? 'coral' : 'plum'} /><Badge tone={priorityClass[item.risk]}>{item.risk}</Badge></div>)}</div><div className="next-panel"><div className="section-heading"><div><span className="micro-label">Siguiente movimiento</span><h3>Para cerrar el primer avance</h3></div><span className="step-count">03</span></div><div className="next-list"><button onClick={() => onNavigate('cases')}><span className="next-check">1</span><span><strong>Completar los datos de prueba</strong><small>Cuentas, saldos y usuarios ficticios</small></span><Icon name="arrow" size={16} /></button><button onClick={() => onNavigate('execution')}><span className="next-check">2</span><span><strong>Ejecutar los 8 casos altos</strong><small>Capturar antes, después y veredicto</small></span><Icon name="arrow" size={16} /></button><button onClick={() => onNavigate('findings')}><span className="next-check">3</span><span><strong>Registrar hallazgos</strong><small>Solo con evidencia, no por suposición</small></span><Icon name="arrow" size={16} /></button></div></div></section>
  </>
}
...
function Plan() {
  return <><PageIntro eyebrow="01 · Planificación" title="Una estrategia corta, trazable y de alto impacto." description="La planificación traduce el riesgo del negocio bancario en una selección concreta de pruebas y datos." /><section className="plan-grid"><div className="plan-main"><div className="plan-callout"><Icon name="shield" size={24} /><div><span className="micro-label">Objetivo de calidad</span><h3>Que ninguna operación crítica pierda consistencia.</h3><p>Validar registro, autenticación, apertura de cuentas, transferencias, bill pay, préstamos, historial y perfil en el sistema real.</p></div></div><div className="plan-section"><div className="section-heading"><div><span className="micro-label">Estrategia</span><h3>Riesgo primero</h3></div><Badge tone="plum">Caja negra</Badge></div><div className="strategy-points"><div><span>01</span><strong>Profundidad en dinero</strong><p>Sistema e integración sobre débito, crédito, saldo e historial.</p></div><div><span>02</span><strong>Datos controlados</strong><p>Cuentas ficticias con saldos conocidos y operaciones identificables.</p></div><div><span>03</span><strong>Evidencia útil</strong><p>Capturas antes/después y registro de resultado para cada caso alto.</p></div></div></div></div><aside className="plan-side"><div className="side-block"><span className="micro-label">Entrada</span><ul><li>URL operativa y navegador estable</li><li>Cuenta de prueba registrada</li><li>Dos cuentas propias con saldo conocido</li><li>Datos ficticios para beneficiario</li></ul></div><div className="side-block"><span className="micro-label">Salida</span><ul><li>100% de requisitos trazados</li><li>Casos altos ejecutados</li><li>Hallazgos con evidencia</li><li>Recomendación para automatizar</li></ul></div><div className="timeline"><span className="micro-label">Ruta de trabajo</span><div><b>Hoy</b><span>Análisis y diseño inicial</span></div><div><b>Próximo</b><span>Ejecución manual crítica</span></div><div><b>Después</b><span>Informe y Proyecto 2</span></div></div></aside></section></>
}

function Traceability() {
  return <><PageIntro eyebrow="02 · Análisis" title="Cada requisito necesita una pregunta comprobable." description="La matriz evita que una funcionalidad importante quede fuera del diseño y muestra por qué algo es prioritario." action={<Badge tone="mint">Cobertura 100%</Badge>} /><div className="table-shell"><div className="table-toolbar"><div><strong>Matriz de trazabilidad</strong><span>10 requisitos · 20 casos relacionados</span></div><div className="toolbar-note"><span className="dot-key high-dot" /> Alto <span className="dot-key medium-dot" /> Medio</div></div><div className="trace-table"><div className="trace-head"><span>ID</span><span>Base de prueba</span><span>Condiciones derivadas</span><span>Riesgo</span><span>Casos</span></div>{requirements.map((item) => <div className="trace-row" key={item.id}><span className="trace-id">{item.id}</span><div><strong>{item.title}</strong><small>{item.type}</small></div><div className="condition-copy">{item.rationale}</div><Badge tone={priorityClass[item.risk]}>{item.risk}</Badge><div className="case-pills">{item.coveredBy.map((id) => <span key={id}>{id}</span>)}</div></div>)}</div></div><div className="analysis-note"><div className="note-icon"><Icon name="spark" size={20} /></div><div><strong>Decisión de análisis</strong><p>Las prioridades altas se concentran en operaciones que pueden modificar dinero o permitir acceso indebido. La búsqueda, perfil y préstamos mantienen cobertura porque sostienen trazabilidad y decisiones posteriores.</p></div></div></>
}
