...
export const initialCases: TestCase[] = [
  { id: 'CP-REG-01', requirement: 'RF-01', feature: 'Registro', condition: 'Crear un cliente con todos los campos obligatorios válidos.', priority: 'Alta', technique: 'Partición de equivalencia', preconditions: 'No existe el nombre de usuario elegido.', data: 'Datos ficticios coherentes; usuario: kalos.demo.01', steps: ['Abrir Registro', 'Completar los campos obligatorios', 'Enviar el formulario'], expected: 'La cuenta se crea y el sistema confirma el registro.', status: 'Pendiente', evidence: false },
  ...
  { id: 'CP-LOG-02', requirement: 'RF-02', feature: 'Login', condition: 'Autenticar con credenciales válidas.', priority: 'Alta', technique: 'Pruebas de caso de uso', preconditions: 'Cuenta de prueba activa.', data: 'Usuario y contraseña válidos.', steps: ['Abrir Login', 'Ingresar credenciales válidas', 'Enviar'], expected: 'El usuario pasa a estado autenticado y ve su resumen de cuentas.', status: 'Pendiente', evidence: false },
  { id: 'CP-ACC-01', requirement: 'RF-03', feature: 'Apertura de cuenta', condition: 'Abrir una cuenta de ahorro fondeada desde una cuenta existente.', priority: 'Alta', technique: 'Tablas de decisión y gráficos causa-efecto', preconditions: 'Cliente autenticado con cuenta de fondeo.', data: 'Tipo: ahorro; cuenta de fondeo válida.', steps: ['Abrir Open New Account', 'Seleccionar Savings', 'Seleccionar cuenta de fondeo', 'Enviar'], expected: 'Se crea la cuenta y aparece en el resumen del cliente.', status: 'Pendiente', evidence: false },
  { id: 'CP-ACC-02', requirement: 'RF-03', feature: 'Apertura de cuenta', condition: 'Intentar abrir una cuenta sin tipo o sin cuenta de fondeo.', priority: 'Alta', technique: 'Tablas de decisión y gráficos causa-efecto', preconditions: 'Cliente autenticado.', data: 'Tipo o cuenta de fondeo sin seleccionar.', steps: ['Abrir Open New Account', 'Omitir un dato requerido', 'Enviar'], expected: 'El sistema impide el envío y comunica qué dato falta.', status: 'Pendiente', evidence: false },
  { id: 'CP-TRF-01', requirement: 'RF-04', feature: 'Transferencia', condition: 'Transferir un monto válido entre dos cuentas propias.', priority: 'Alta', technique: 'Tablas de decisión y gráficos causa-efecto', preconditions: 'Dos cuentas propias y saldo suficiente.', data: 'Monto: 100.00; origen y destino distintos.', steps: ['Abrir Transfer Funds', 'Seleccionar origen y destino', 'Ingresar 100.00', 'Enviar', 'Consultar saldos'], expected: 'El origen disminuye 100.00, el destino aumenta 100.00 y la transacción aparece en el historial.', status: 'Pendiente', evidence: false },
  { id: 'CP-TRF-02', requirement: 'RF-04', feature: 'Transferencia', condition: 'Transferir un monto superior al saldo disponible.', priority: 'Alta', technique: 'Análisis de valores límite', preconditions: 'Cuenta de origen con saldo conocido.', data: 'Monto: saldo + 0.01.', steps: ['Abrir Transfer Funds', 'Seleccionar origen y destino', 'Ingresar saldo + 0.01', 'Enviar'], expected: 'La operación se rechaza y no se modifica ningún saldo.', status: 'Pendiente', evidence: false },
  { id: 'CP-TRF-03', requirement: 'RF-04', feature: 'Transferencia', condition: 'Transferir cero, un monto negativo o usar la misma cuenta como origen y destino.', priority: 'Alta', technique: 'Análisis de valores límite', preconditions: 'Cliente autenticado.', data: 'Monto: 0.00 y -1.00; origen = destino.', steps: ['Abrir Transfer Funds', 'Probar cada dato inválido', 'Enviar'], expected: 'El sistema rechaza los datos inválidos sin alterar saldos.', status: 'Pendiente', evidence: false },
  { id: 'CP-BP-01', requirement: 'RF-05', feature: 'Bill pay', condition: 'Pagar un servicio con beneficiario, cuenta y monto válidos.', priority: 'Alta', technique: 'Tablas de decisión y gráficos causa-efecto', preconditions: 'Cuenta con saldo suficiente.', data: 'Beneficiario ficticio; monto: 25.00.', steps: ['Abrir Bill Pay', 'Completar datos del beneficiario', 'Seleccionar cuenta', 'Ingresar 25.00', 'Enviar'], expected: 'El pago se confirma, descuenta 25.00 y registra una operación consultable.', status: 'Pendiente', evidence: false },
  { id: 'CP-BP-02', requirement: 'RF-05', feature: 'Bill pay', condition: 'Enviar un pago sin nombre, dirección, cuenta o monto.', priority: 'Alta', technique: 'Partición de equivalencia', preconditions: 'Cliente autenticado.', data: 'Omitir un campo requerido por iteración.', steps: ['Abrir Bill Pay', 'Dejar un dato vacío', 'Enviar'], expected: 'El sistema identifica el dato faltante y no procesa el pago.', status: 'Pendiente', evidence: false },
  { id: 'CP-BP-03', requirement: 'RF-05', feature: 'Bill pay', condition: 'Pagar un monto mayor al saldo disponible.', priority: 'Alta', technique: 'Análisis de valores límite', preconditions: 'Cuenta con saldo conocido.', data: 'Monto: saldo + 0.01.', steps: ['Abrir Bill Pay', 'Completar beneficiario', 'Ingresar saldo + 0.01', 'Enviar'], expected: 'El pago se rechaza y el saldo permanece intacto.', status: 'Pendiente', evidence: false },
  { id: 'CP-HIS-01', requirement: 'RF-06', feature: 'Historial', condition: 'Buscar operaciones por fecha y rango de fechas.', priority: 'Alta', technique: 'Partición de equivalencia', preconditions: 'Existen transacciones de prueba.', data: 'Fecha exacta y rango que incluya una operación conocida.', steps: ['Abrir Account History', 'Consultar una fecha', 'Consultar un rango'], expected: 'Solo se muestran las operaciones que corresponden al filtro.', status: 'Pendiente', evidence: false },
  { id: 'CP-HIS-02', requirement: 'RF-06', feature: 'Historial', condition: 'Buscar una operación por monto o ID de transacción.', priority: 'Alta', technique: 'Partición de equivalencia', preconditions: 'Existe una transacción con datos conocidos.', data: 'Monto exacto e ID obtenido después de una operación.', steps: ['Abrir búsqueda de transacciones', 'Ingresar monto', 'Repetir con ID'], expected: 'El resultado identifica la operación correcta sin mezclar registros.', status: 'Pendiente', evidence: false },
  { id: 'CP-LOAN-01', requirement: 'RF-07', feature: 'Préstamos', condition: 'Solicitar un préstamo con enganche suficiente para aprobación.', priority: 'Alta', technique: 'Tablas de decisión y gráficos causa-efecto', preconditions: 'Cliente autenticado y formulario disponible.', data: 'Monto solicitado y down payment que cumpla el umbral configurado.', steps: ['Abrir Request Loan', 'Ingresar monto y enganche', 'Enviar'], expected: 'El sistema muestra una aprobación explícita.', status: 'Pendiente', evidence: false },
  { id: 'CP-LOAN-02', requirement: 'RF-07', feature: 'Préstamos', condition: 'Solicitar un préstamo con enganche insuficiente.', priority: 'Alta', technique: 'Análisis de valores límite', preconditions: 'Cliente autenticado.', data: 'Down payment por debajo del umbral.', steps: ['Abrir Request Loan', 'Ingresar monto y enganche insuficiente', 'Enviar'], expected: 'El sistema muestra un rechazo explícito y no aprueba el préstamo.', status: 'Pendiente', evidence: false },
  { id: 'CP-PROF-01', requirement: 'RF-08', feature: 'Perfil', condition: 'Actualizar datos de contacto y comprobar persistencia.', priority: 'Alta', technique: 'Pruebas de caso de uso', preconditions: 'Cliente autenticado.', data: 'Teléfono y dirección ficticios válidos.', steps: ['Abrir Update Contact Info', 'Modificar los datos', 'Guardar', 'Salir y volver al perfil'], expected: 'Los datos actualizados permanecen visibles en el perfil.', status: 'Pendiente', evidence: false },
  { id: 'CP-NFR-01', requirement: 'RNF-01', feature: 'Rendimiento', condition: 'Medir el tiempo de respuesta de una transferencia o pago.', priority: 'Alta', technique: 'Pruebas estadísticas', preconditions: 'Cuenta de prueba y cronómetro disponibles.', data: 'Transacción válida de bajo monto.', steps: ['Iniciar cronómetro al enviar', 'Detener al recibir confirmación', 'Registrar el tiempo'], expected: 'La confirmación llega en menos de 3 segundos.', status: 'Pendiente', evidence: false },
  { id: 'CP-NFR-02', requirement: 'RNF-02', feature: 'Sesión', condition: 'Verificar el comportamiento después de inactividad.', priority: 'Alta', technique: 'Tablas de decisión y gráficos causa-efecto', preconditions: 'Sesión iniciada; ventana de inactividad definida por el entorno.', data: 'Sin interacción durante el intervalo observado.', steps: ['Iniciar sesión', 'No interactuar', 'Intentar abrir una función protegida'], expected: 'La sesión se cierra y el sistema exige autenticación nuevamente.', status: 'Pendiente', evidence: false },
]
